...
export class WorkerRepository implements IWorkerRepository {
  async findById(workerId: string): Promise<Worker | null> {
    const worker = await WorkerModel.findOne({ _id: workerId }).populate(
      "service",
      "name"
    );
  ...
  async getWorker(
    ...
    pageNumber: number,
    pageSize: number
  ): Promise<{ workers: Worker[]; totalPages: number }> {
  ...
  async toggleReachStatus(
    bookingId: string,
    reachStatus: string
  ): Promise<void> {
    await bookingModel.findByIdAndUpdate(bookingId, {
      reachingStatus: reachStatus,
    });
  }
  ...
  async getWallet(workerId: string): Promise<Wallet | null> {
    ...
    // Sort walletHistory by date (newest first)
    wallet.walletHistory.sort((a, b) => b.date.getTime() - a.date.getTime());
    await wallet.save();
